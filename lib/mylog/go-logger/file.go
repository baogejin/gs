...
import (
	"errors"
	"gs/lib/mylog/go-logger/log_utils"
	"os"
	"path"
	"reflect"
	"strings"
...
func (adapterFile *AdapterFile) Init(fileConfig Config) error {
	...
	_, ok := fileSliceDateMapping[strings.ToLower(adapterFile.config.DateSlice)]
	if !ok && adapterFile.config.DateSlice != FILE_SLICE_DATE_NULL {
...
func (fw *FileWriter) initFile() error {
	...
	ok, _ := log_utils.UtilFile.PathExists(fw.filename)
	if ok == false {
		err := log_utils.UtilFile.CreateFile(fw.filename)
		if err != nil {
			return err
		}
	}
	...
	nowLines, err := log_utils.UtilFile.GetFileLines(fw.filename)
	if err != nil {
		return err
	}
